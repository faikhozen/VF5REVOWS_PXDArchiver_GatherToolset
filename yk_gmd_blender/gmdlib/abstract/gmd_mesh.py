...
class GMDSkinnedMesh(GMDMesh):
    ...
    def __post_init__(self):
        super().__post_init__()
        # Convert bone_data to signed int to avoid wrapping -1 to 255 under NumPy 2.0 type promotion rules
        bone_data_signed = self.vertices_data.bone_data.astype(np.int32)
        referenced_bone_indices = set(int(x) for x in np.unique(
            np.where(self.vertices_data.weight_data > 0, bone_data_signed, -1)).flatten())
